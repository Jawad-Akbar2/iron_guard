...
export const SupplierLedgerModal = ({ data }) => {
  ...
  const handleDeleteTransaction = async (txnId) => {
    if (window.confirm('Delete this transaction? Stock will be reversed.')) {
      try {
        await supplierAPI.softDeleteTransaction(txnId);
  ...
  const handleExportCSV = () => {
    ...
    const csv = [
      ['Transaction ID', 'Date & Time', 'Item', 'Quantity', 'Purchase Rate', 'Discount', 'Total', 'Payment Type', 'Paid', 'Balance', 'Profit'],
    ...
    a.href = url;
    a.download = `supplier-ledger-${Date.now()}.csv`;
    a.click();
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-6xl max-h-96 overflow-y-auto">
        <div className="sticky top-0 bg-gradient-to-r from-orange-600 to-orange-800 text-white p-6 flex justify-between items-center">
          ...
          <button
            onClick={() => closeModal('supplierLedger')}
              ...
              <tbody className="divide-y">
                {transactions.map((txn) => (
                  <tr key={txn._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 text-right text-xs font-medium text-orange-600">{formatCurrency(txn.remainingBalance)}</td>
